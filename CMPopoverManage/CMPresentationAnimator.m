...
CMPresentationAnimator *animator;

@interface CMPresentationAnimator ()
/** 是否是弹出状态 */
...
+ (instancetype)presentationAnimator {
    ...
    dispatch_once(&onceToken, ^{
        animator = [[CMPresentationAnimator alloc] init];
        animator.animationorType = CMPresentationAnimatorTypeDefault;
    });
    return animator;
}
...
- (void)animationForPresentedView:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    //动画类型
    if (animator.animationorType == CMPresentationAnimatorTypeDefault) {
        [transitionContext completeTransition:YES];
        
    } else if (animator.animationorType == CMPresentationAnimatorTypeTranslation) {
